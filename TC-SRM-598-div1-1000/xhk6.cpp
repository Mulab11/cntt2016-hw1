...
int n,ans;
int f[60][60],ga[60][60]; //这里为了实现方便使得空间复杂度变成了O(n^2),可以通过把f变成一维的，树用邻接表存的方式把空间复杂度降到O(n) 
...
{int j,s=0,flag=0;
 ...
 f[i][fa]=0;
 for(j=1;j<=n;j++) //这段转移方程的写法采用了题解里面的第二种 
 	if(j!=fa && ga[i][j])
...
{	public:
	...
	{	int i,j;
		...
		for(i=1;i<=n;i++) //枚举root 
		{	dfs(i,0);
			ans=min(ans,f[i][0]+1); //更新答案 
		}
		return ans;
	}
