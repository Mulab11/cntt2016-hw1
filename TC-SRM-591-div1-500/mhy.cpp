...
class PyramidSequences {
    public:
        long long distinctPairs(int n, int m) {
            long long g=gcd(n-1,m-1),x=(n-1)/g,y=(m-1)/g;
            return x*y*(g-1)+((x+1)*(y+1)+1)/2;
        }
};

// CUT begin
ifstream data("PyramidSequences.sample");
...
bool do_test(int N, int M, long long __expected) {
