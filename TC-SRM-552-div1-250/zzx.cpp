...
bool check(LL x) {
    ...
    if (num % 3 == 0) {
        num /= 3;
        if (r / num >= x && g / num >= x && b / num >= x) return true;  //如果num是3的倍数，那么每种颜色需要的数量相同
    } else {
        num /= 3;
        if (!num || (r / num >= x && g / num >= x && b / num >= x)) { 
            if (r - x * num + g - x * num + b - x * num >= x) return true; //不然还需要满足每堆至少要有一个多的
        }
    }
    return false;
}
