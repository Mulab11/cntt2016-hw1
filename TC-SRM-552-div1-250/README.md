...
进一步观察可得：如果$$N$$为偶数，则排列成一个三角形所用的三种小球数总是满足其中一种颜色比另外两种颜色多用$$1$$个，否则所用三种小球数目相等，且均为$${n*(n+1)/6}$$个。
...
如果$$N$$为偶数，则三种颜色个数应均大于等于$${n*(n+1)}/6$$，且三种颜色减去$${n*(n+1)}/6$$剩下的个数之和应大于等于答案。

如果$$N$$为奇数，则三种颜色个数应均大于等于$${n*(n+1)}/6$$。
