...
const int N = 1700 + 5, M = N * 40 * 2;
struct Edge{ int nxt, to, flow; } e[M];
int num[N], sta[N], hd[N], tail, s, t, q[N], pos[50 + 5][50 + 5];
...
{  
        ...
        {  
        	 //建立最小割模型，每个点拆点k = 1..n 表示该点与S相连代表是否>=k。 
        	 int n = A.size(), tot = 0;
        	 REP(i, 1, n) REP(j, 0, n) pos[i][j] = ++tot;
        	 
        	 MEM(hd, 0), s = 0, t = tot + 1, tail = 1;
			 REP(i, 1, n) REP(j, 0, n - 1) add(pos[i][j], pos[i][j + 1], ((i == 1) ^ (j == 0) ? inf : (j - B[i - 1]) * (j - B[i - 1])));
        	 REP(i, 1, n) add(s, pos[i][0], inf), add(pos[i][n], t, inf);
        	 REP(i, 1, n) REP(j, 1, n) if (A[i - 1][j - 1] == 'Y')
        	 	REP(k, 1, n) add(pos[i][k], pos[j][k - 1], inf);
