...
class SpaceWarDiv1 {
    public:
        ...
        long long minimalFatigue(vector<int> magicalGirlStrength, vector<int> enemyStrength, vector<long long> enemyCount) {
            ...
            for (int i=0;i<m;i++) a.push_back(make_pair(enemyStrength[i],enemyCount[i]));
            sort(a.rbegin(),a.rend());
            long long ans=0,sum=0;
            for (int i=0;i<m;i++){
                sum+=a[i].second;
                int c=0;
                for (int j=0;j<n;j++) if (magicalGirlStrength[j]>=a[i].first) c++;
                ans=max(ans,c?(sum+c-1)/c:INF);
            }
