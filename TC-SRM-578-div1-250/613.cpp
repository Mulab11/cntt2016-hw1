...
using namespace std;
vector <int> des[200100];
bool vis[200100];
int dfs(int s)
{	
	vis[s]=true;
	int ans=1;
	for (int k=0;k<des[s].size();k++) if (!vis[des[s][k]]) ans+=dfs(des[s][k]);
	return ans;
}
...
class GooseInZooDivOne
{
        ...
        int count(vector <string> a,int d)
        {
			...
			{
				if ((a[px][py]!='v')||(a[qx][qy]!='v')) continue;
				if ((px==qx)&&(py==qy)) continue;
				if (abs(qx-px)+abs(qy-py)>d) continue;
				des[px*m+py].push_back(qx*m+qy);
				//des[qx*m+qy].push_back(px*m+py);
			}
        	int even=0,odd=0;
        	for (int i=0;i<n;i++)
			for (int j=0;j<m;j++)
			{
				if (a[i][j]!='v') continue;
				int w=i*m+j;
				if (!vis[w]) {if (dfs(w)&1) odd++;else even++;}
			}
        	cout<<odd<<' '<<even<<endl;
        	return (getmi(2,max(odd,1)-1+even)+mo-1)%mo;
    	}
