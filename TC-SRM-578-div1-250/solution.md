...
对于当前点，设它为原点，用$$x$$轴，$$y$$轴，直线$$y=x$$，直线$$y=-x$$将整张图分为16个部分（8个平面区域和8个射线区域）。将这个点和每个区域中离它最近的点连一条边，这张图的连通性和原图是等价的。下面从搜索的角度来简单地证明一下：

考虑其中某一个区域，其它的可以类比来论证。

![图片无法显示QAQ](pics/pic1.jpg)

红色的虚线段上的点和原点距离相等，假设红线上的点$$A$$是离$$O$$最近的一个点，那么如果红线上有多个点显然任取一个都可以（其它的点会被$$A$$给搜索到）。

考虑某个点$$P$$，红线外的点$$P$$（红线内没有点），$$PA$$之间的距离是严格小于$$PO$$之间的距离的。则对于某个和$$O$$距离不超过$$dist$$的点，它要么已经被搜过了，要么是最近的点之一，要么存在一个点缩短了距离。因此$$P$$一定会被搜到。
