...
$$前面一项表示$$i$$是红点和蓝点时的情况，后面一项表示$$i$$是白点时的情况。用多项式乘法直接计算即可。

因为之前并没有限定根是谁，所以我们就定编号为$$k$$的白点为根。枚举哪一个结点编号为$$k$$，显然红点数和蓝点数都是$$n-k$$。因为之前的转移方程没有考虑白点的编号，所以答案要乘上$$(n-2k-1)!$$。所以答案就是$$(n-2k-1)!\sum_{i=1}^{n}F[i][0][n-k][n-k]$$。如果计算多项式乘法时暴力计算的话时间复杂度是$$