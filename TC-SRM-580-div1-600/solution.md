...
这样的话我们可以从 $$i$$ 开始 BFS, 找到 $$i$$ 到 $$l$$ 和 $$r$$ 的最短传播距离, 然后用 $$\max(dist(i, l) - 1, 0) + \max(dist(i, r) - 1, 0)$$ 来更新 $$f[i]$$.
...
注意到连边是双向的, 因此我们不需要 BFS $$n$$ 次, 而只需要从 $$l$$ 和 $$r$$ 开始各 BFS 各一次就可以求出 $$dist(i, l)$$ 与 $$dist(i, r)$$ 了, 这样的时间复杂度是 $$O(n^2)$$ 的.
