...
class Egalitarianism {
    public:
        ...
        int maxDifference(vector<string> isFriend, int d) {
            ...
            for (int i=0;i<n;i++) for (int j=0;j<n;j++) if (isFriend[i][j]=='Y') dist[i][j]=1;
            int ans=0;
            for (int k=0;k<n;k++) for (int i=0;i<n;i++) for (int j=0;j<n;j++) tension(dist[i][j],dist[i][k]+dist[k][j]);
            for (int i=0;i<n;i++) for (int j=0;j<n;j++) relax(ans,dist[i][j]);
            return ans!=0x3f3f3f3f?ans*d:-1;
        }
};
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "Egalitarianism (250 Points)" << endl << endl;
    }
