...
class Egalitarianism {
	public:
		int maxDifference(vector<string> isFriend, int d) {
            n = isFriend.size();
            REP(i, 0, n - 1)
                REP(j, 0, n - 1) a[i][j] = 1000;
            REP(i, 0, n - 1) a[i][i] = 0;
            REP(i, 0, n - 1)
                REP(j, 0, n - 1) if (isFriend[i][j] == 'Y') a[i][j] = 1;
            REP(k, 0, n - 1)
                REP(i, 0, n - 1)
                    REP(j, 0, n - 1) getmin(&a[i][j], a[i][k] + a[k][j]); //计算两两之间最短路
            int mx = 0;
            REP(i, 0, n - 1) REP(j, 0, n - 1) getmax(&mx, a[i][j]); 
            if (mx == 1000) return -1;
			return mx * d;
		}
};
