...
## 题目简述
给你一个栈，里面有无穷多个数，都是$$0$$。
对于一个包含$$n$$个非负整数操作序列$$program$$，依次对这个栈进行操作。
如果当前的数大于$$0$$，就把该数压进栈里。
如果当前的数为$$0$$，那么就把栈顶两个数弹出，并把这两个数的和压进栈里。
由于一些信息缺失，给你的操作序列里面有且仅有一个数是未知的，用$$-1$$表示，它可以是任意非负整数。
但现在告诉你所有操作完后的栈顶的数$$wantedResult$$，问你未知数解的情况（无解，一个解，或无穷解）
...
我们不妨进行分类：
  ...
  2、**未知数不为0。** 不难发现栈顶的数是一些数的和，那我们模拟压栈弹栈的过程便可得到哪些数组成了最后的答案，最终得到这样一个方程$$ax=b，a=0/1$$。那么剩下的就是解一元一次方程的事情了。关于$$a$$的系数，我们又可以做如下分类（~~这种小学生都会得东西你也要写？~~）：
