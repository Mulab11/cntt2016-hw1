...
LL eval(int x) { //dsf 
    stack<LL> S; 
    REP(i, 1, n + 1) S.push(0);
    REP(i, 1, n) {
        int j = a[i]; if (j == -1) j = x;
        if (!j) {
            LL t1 = S.top(); S.pop();
            LL t2 = S.top(); S.pop();
            S.push(t1 + t2);
...
class Suminator {
	public:
		int findMissing(vector<int> program, int wantedResult) {
            n = program.size(); //hwhwhw
            REP(i, 1, n) a[i] = program[i - 1];
            LL x0 = eval(0), x1 = eval(1), x2 = eval(2);
            if (x0 == wantedResult) return 0;
            if (x1 == wantedResult) return 1;
            if (x1 == x2) return -1;
            if (x1 > wantedResult) return -1;
			return wantedResult - x1 + 1;
		}
};
