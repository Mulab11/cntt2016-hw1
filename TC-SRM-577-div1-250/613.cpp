...
class EllysRoomAssignmentsDiv1
{
        ...
        {
			string s;
			for (int i=0;i<rat.size();i++) s+=rat[i];
			vector <int> vec;
			for (int i=0;i<s.size();i+=5) vec.push_back((s[i]-'0')*1000+(s[i+1]-'0')*100+(s[i+2]-'0')*10+(s[i+3]-'0'));
			int u=vec[0];
			sort(vec.begin(),vec.end(),cmp);
			//for (int i=0;i<vec.size();i++) cout<<vec[i]<<endl;
			int n=vec.size();
			int r=n/20+(n%20!=0);
			double ans=0,pos=-1,sum;
			for (int i=0;i<vec.size();i+=r)
			{
				int d=-1;sum=0;
				for (int j=i;j<min((int)vec.size(),i+r);j++)
				{
					if (vec[j]==u) d=pos=j;
					sum+=vec[j];
				}
				if (d!=-1) continue;
				if (i+r>vec.size()) break;
				ans+=sum/r;
			}
			ans+=u;
			if (n%r==0) {printf("1\n");return ans/(n/r);}
			else if (pos>=n-n%r) {printf("2\n");return ans/(n/r+1);}
        	else
			{
				printf("3\n");
				return (ans/(n/r)*((-n)%r+r)+(ans+sum/(n%r))/(n/r+1)*(n%r))/r;
			}
