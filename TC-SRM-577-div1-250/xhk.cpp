...
{	
	...
	{ 
		...
		//找出所有rate
		string ratesString=accumulate(ratings.begin(),ratings.end(),string());
		stringstream ss(ratesString);
		while(ss>>i) reg[n++]=i;
		//Elly在第一个
		int elly=reg[0];
		//降序排列
		...
		//找出Elly的位置
		int ep=(int)(find(reg,reg+n,elly)-reg);
		...
		{	//每次随机拍R个人，最后一次可能<R
			...
			if(j<=0) break;
			//R个人的期望值
			double avg=(double)accumulate(reg+i*R,reg+i*R+j,0)/(double)j;
			//Elly所在R个人的期望值为Elly自己
			if(ep>=i*R && ep<((i+1)*R))
				avg=elly;
			else if(j<R)
			{	//如果n=32，elly可能在10人间，概率1/3；也可能在11人间，概率2/3
				double qw=((sum+avg)/(i+1))*j/R+(sum/i)*(R-j)/R; 
				return qw;
			}
			sum+=avg;
		}
