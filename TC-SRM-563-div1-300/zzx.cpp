...
bool check(string a, string b) {
    ...
    for (int i = 0; i < b.size(); ++i) {//greedy
        if (j < a.size() && a[j] == b[i]) y[a[j]]++, ++j;
        else if (j >= a.size()) z[b[i]]++;
        x[b[i]]++;
    }
    if (j < a.size()) return false;
    REP(i, 'a', 'z') if (z[i] + y[i] < x[i] / 2 || y[i] * 2 > x[i]) return false;
    return true;
}
...
class FoxAndHandle
{
        ...
        {
            string ans;
            for (int i = 0; i < S.size() / 2; ++i) { //try each position
                ans.PB('a');
                REP(j, 'a', 'z') {
                    ans[i] = j;
                    if (check(ans, S)) break; // check if the prefix is valid
                }
            }
            return ans;
        }
