...
/*======================= TEMPLATE =======================*/
const int N = 200000;
LL fact[N], inv[N];//预处理阶乘和阶乘的逆
LL C(int n, int r) { // 计算组合数
    return fact[n] * inv[r] % mo * inv[n - r] % mo;
}
int x[N], y[N];
int n;
...
LL f(int *x, int m) {
    LL ret = 0;
    for (int tx = -300000; tx <= 300000; tx++) {//枚举所有点在每个维度上集合的地点
        LL mult = 1;
        for (int i = 0; i < n; i++) { //暴力去计算方案数
            int d = abs(tx-x[i]);
            if (d > m || (m+d)%2 == 1) {
                mult = 0;
                break;
...
public:
    int calc(vector <int> xx, vector <int> yy, int m) {
        ...
        REP(i, 1, N - 1) { //预处理
            ...
            inv[i] = Pow(fact[i], mo - 2);
        }
        n = xx.size();
        REP(i, 0, n - 1) 
            x[i] = xx[i] + yy[i], y[i] = xx[i] - yy[i]; //将坐标轴旋转45度 这样两个维度就互相独立了 
        LL ans = f(x, m) * f(y, m) % mo;//因为两个维度互相独立了 所以就可以直接乘起来了
        return ans;
    }
};
