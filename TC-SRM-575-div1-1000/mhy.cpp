...
class TheTilesDivOne {
    public:
        ...
        int find(vector<string> board) {
            ...
            int nn=board.size(),mm=board[0].size();
            V=0,S=++V,T=++V;
            for (int i=0;i<nn;i++) for (int j=0;j<mm;j++) if (board[i][j]!='X') id[0][i][j]=++V,id[1][i][j]=++V;
            for (int i=0;i<nn;i++) for (int j=0;j<mm;j++) if (board[i][j]!='X'){
                add_double_edge(id[0][i][j],id[1][i][j],1);
                for (int f=0;f<4;f++){
                    int x=i+dx[f],y=j+dy[f];
                    if (x<0||x>=nn||y<0||y>=mm||board[x][y]=='X') continue;
                    add_double_edge(id[1][i][j],id[0][x][y],1);
                }
                if ((i+j)&1){
                    if (~j&1) add_double_edge(S,id[0][i][j],1); else add_double_edge(id[1][i][j],T,1);
                }
            }
