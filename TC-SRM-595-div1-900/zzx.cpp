...
LL area(int i, int j, int k) {
    return 1LL * (x[i] - x[k]) * (y[j] - y[k]) - 1LL * (x[j] - x[k]) * (y[i] - y[k]);
}
bool inside(int a, int b, int c) {
    if (x[a] == x[b]) 
        return y[c] <= max(y[a], y[b]) && y[c] >= min(y[a], y[b]);
        return x[c] <= max(x[a], x[b]) && x[c] >= min(x[a], x[b]);
}
double check(int a, int b) {
    double ans = 1;
    REP(i, 1, n) if (i != a && i != b) {
        LL tmp = area(i, b, a);
        if (tmp < 0 || (tmp == 0 && inside(a, b, i)))  ans *= (1.0 - p[i]);
    }
    return ans;
}
class Constellation {
	public:
		double expectation(vector<int> X, vector<int> Y, vector<int> prob) {
            n = X.size();
            REP(i, 1, n) x[i] = X[i - 1], y[i] = Y[i - 1], p[i] = 1.0 * prob[i - 1] / 1000;
            double ans = 0;
            REP(i, 1, n) REP(j, 1, n) if (i != j) {
                ans += 0.5 * area(j, i, 0) * p[i] * p[j] * check(i, j);
            }
			return abs(ans);
		}
};
