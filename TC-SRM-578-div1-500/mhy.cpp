...
class WolfInZooDivOne {
    public:
        ...
        int count(int n, vector<string> L, vector<string> R) {
            ...
            for (int l,r;isl>>l,isr>>r;) a.push_back(make_pair(l,r));
            int m=a.size();
            for (int i=0;i<n;i++) for (int j=i+1;j<=n;j++){
                dp[i][j]=i?dp[i-1][j]:1;
                int now=i;
                for (int k=0;k<=m;k++) if (a[k].first<=i&&j<=a[k].second) now=min(now,a[k].first);
                inc(dp[i][j],now?dp[now-1][i]:1);
            }
            return dp[n-1][n];
        }
};
