...
class TomekPhone {
    public:
        int minKeystrokes(vector<int> frequencies, vector<int> keySizes) {
            vector<int> a;
            int n=frequencies.size(),m=keySizes.size();
            for (int i=0;i<m;i++) for (int j=1;j<=keySizes[i];j++) a.push_back(j);
            if (a.size()<n) return -1;
            sort(a.begin(),a.end());
            sort(frequencies.rbegin(),frequencies.rend());
            int ans=0;
            for (int i=0;i<n;i++) ans+=frequencies[i]*a[i];
            return ans;
        }
};
...
int main(int argc, char *argv[]) {
    ...
    if (mainProcess) {
        cout << "TomekPhone (250 Points)" << endl << endl;
    }
