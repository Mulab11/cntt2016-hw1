...
class FoxAndMountainEasy {
    ...
    string possible(int n, int h0, int hn, string history) {
        for (int i=0;i<(int)history.size();n--,i++) if (history[i]=='U') hn--; else hn++;
        hn-=h0;
        printf("%d %d\n",n,hn);
        if (!(hn+n>=0&&(hn+n)%2==0&&n-hn>=0&&(n-hn)%2==0)) return "NO";
        int x=(hn+n)>>1;
        for (int now=h0+x,i=0;i<(int)history.size();i++){
            if (now<0) return "NO";
            if (history[i]=='U') now++; else now--;
        }
        return "YES";
    }
};
