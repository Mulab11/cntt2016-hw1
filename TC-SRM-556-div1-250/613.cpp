...
bool f[55][1024];
int ans;
void search(int s,int w)
{
	//cout<<s<<' '<<w<<endl;
	f[s][w]=true;ans=max(ans,w);
	for (int i=0;i<vec.size();i++) if ((bo[s][i]=='Y')&&(!f[i][w^vec[i]])) search(i,w^vec[i]);
}
class XorTravelingSalesman
{
        public:
        int maxProfit(vector <int> vec_inp, vector <string> bo_inp)
        {
        	vec=vec_inp;bo=bo_inp;
