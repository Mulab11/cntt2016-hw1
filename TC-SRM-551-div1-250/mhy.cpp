...
class ColorfulChocolates {
    public:
        ...
        int maximumSpread(string chocolates, int maxSwaps) {
            ...
            for (char c='A';c<='Z';c++){
                int m=0;
                for (int i=0;i<n;i++) if (chocolates[i]==c) a[m++]=i;
                for (int k=0;k<m;k++){
                    pre[k]=0;
                    for (int i=k-1;~i;i--) pre[i]=pre[i+1]+(a[k]-(k-i)-a[i]);
                    suf[k]=0;
                    for (int i=k+1;i<m;i++) suf[i]=suf[i-1]+(a[i]-(i-k)-a[k]);
                    for (int i=0;i<=k;i++) for (int j=k;j<m;j++) if (pre[i]+suf[j]<=maxSwaps) ans=max(ans,j-i+1);
                }
            }
            return ans;
        }
