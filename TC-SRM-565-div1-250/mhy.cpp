...
class MonstersValley {
    public:
        ...
        int minimumPrice(vector<long long> dread, vector<int> price) {
            ...
            int n=dread.size(),m=200;
            dp[0][0]=0;
            for (int i=0;i<n;i++) for (int j=0;j<m;j++) if (~dp[i][j]){
                relax(dp[i+1][j+price[i]],dp[i][j]+dread[i]);
                if (dp[i][j]>=dread[i]) relax(dp[i+1][j],dp[i][j]);
            }
            for (int i=0;;i++) if (~dp[n][i]) return i;
        }
};
