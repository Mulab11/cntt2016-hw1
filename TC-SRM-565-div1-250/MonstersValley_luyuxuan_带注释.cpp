...
#include <ctime>
using namespace std;
long long f[55][110];//f[i][j]表示走到第i个位置受到至多j点伤害时的最高智商 
class MonstersValley
{
        ...
        {
            int n=price.size();
			memset(f,214,sizeof(f));//把f数组全部赋值为"无解" 
			for (int i=price[0];i<=2*n;i++) f[0][i]=dread[0];//必须打第一个怪因为智商是0 
			for (int i=1;i<n;i++)
			for (int j=1;j<=2*n;j++)
			{
				f[i][j]=f[i][j-1];//少受1点伤害
				if (j-price[i]>0) f[i][j]=max(f[i][j],f[i-1][j-price[i]]+dread[i]);//来干 
				if (f[i-1][j]>=dread[i]) f[i][j]=max(f[i][j],f[i-1][j]);//怂逼 
			}
