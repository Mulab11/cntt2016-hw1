...
class BoardPainting {
    public:
        ...
        int minimalSteps(vector<string> target) {
            ...
            for (int i=0;i<n;i++) for (int j=0;j<m;j++) if (target[i][j]=='#') id[i][j]=++V;
            for (int c,i=0;i<n;i++) for (int j=0;j<m;j++) if (target[i][j]=='#'){
                for (int f=0;f<4;f++){
                ...
                for (int f=0;f<4;f+=2){
                    int x=i+dx[f],y=j+dy[f];
                    if (x<0||x>=n||y<0||y>=m) c++;
                    else if (target[x][y]!='#') c++;
                }
                add_double_edge(S,id[i][j],c);
                c=0;
                for (int f=1;f<4;f+=2){
                    int x=i+dx[f],y=j+dy[f];
                    if (x<0||x>=n||y<0||y>=m) c++;
                ...
                add_double_edge(id[i][j],T,c);
            }
            return Dinisap.max_flow()>>1;
        }
};
